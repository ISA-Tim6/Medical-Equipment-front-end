...
export class CompanyService {
  ...
  addEquipment(equipment:Equipment,id:number):Observable<any>{
    return this.http.put<any>(environment.apiHost+"company/addEquipment/"+id,equipment);

  }

  removeEquipment( equipment:Equipment,company_id:number):Observable<any>{
    return this.http.put<any>(environment.apiHost+"company/removeEquipment/"+company_id,equipment);

  }

  updateCompany(companyUpdate:CompanyUpdate):Observable<any>{
    return this.http.put<any>(environment.apiHost+"company/"+companyUpdate.company_id,companyUpdate);
  }

  getOtherCompanyAdminsForCompany(company_id:number,user_id:number):Observable<CompanyAdmin[]>{
    return this.http.get<any>(environment.apiHost+"companyAdmin/"+company_id+"/"+user_id);
