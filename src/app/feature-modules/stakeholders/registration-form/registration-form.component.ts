...
import { Employment, RegistratedUser } from '../model/user.model';
@Component({
...
export class RegistrationFormComponent implements OnChanges {
  ...
  user: RegistratedUser = {
    ...
    infoAboutInstitution:
      this.registrationForm.value.infoAboutInstitution || '',
    loggedBefore: false,
  };

  public registerUser() {
    this.user.infoAboutInstitution =
      this.registrationForm.get('infoAboutInstitution')!.value || '';
  ...
  isPhoneNumberValid() {
    ...
    if (sampleRegEx.test(this.user.phoneNumber))
      if (this.user.phoneNumber.length > 3) isValidPhoneNumber = true;
  ...
  isAllValid() {
    ...
    if (this.isValid(this.user.name)) nameValid = true;

    if (this.isValid(this.user.email)) surnameValid = true;
